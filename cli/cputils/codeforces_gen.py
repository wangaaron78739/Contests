...
def main(cases: bool, filenames: list[str]):
        ...
        try:
            template_file = (
                pkg_resources.resource_filename(
                    __name__, "../../libs/cf_template_cases.cpp"
                )
                if cases
                else pkg_resources.resource_filename(
                    __name__, "../../libs/cf_template.cpp"
                )
            )
            for filename in [f + ".cpp" for f in filenames]:
                _ = shutil.copyfile(template_file, filename)
                panel.add(f"Generated {filename}{' with cases' if cases else ''}.")
            panel.add("Done.")
            if not os.path.exists("samples"):
                panel.renderable += "Created samples directory.\n"
                os.makedirs("samples")
